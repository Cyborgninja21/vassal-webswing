...
The repo builds **two images**:

| Image | Built from | What it is |
|---|---|---|
| `ghcr.io/cyborgninja21/vassal-webswing` | `docker/`, `src/`, `patches/` | the engine — Webswing Lite + VASSAL + the header-trust security realm |
| `ghcr.io/cyborgninja21/vassal-portal` | [`portal/`](portal/) | the place — game catalog and live table browser, served at `/` on the same hostname |

They ship together as one stack and are versioned together, but have separate CI
workflows so a catalog tweak does not rebuild the ~2 GB engine image.
...
| `docker/icons/*.png` | 256x256 module tiles for Webswing's app-selection dialog (`SecuredPathConfig.icon`), generated by `tools/make-icons.py`. Original artwork — no publisher box art is redistributed. |
| `tools/make-icons.py` | Deterministic PIL generator for the above; shares its palette and motifs with the portal's catalog tiles. |
| `portal/` | The Next.js portal: catalog + live table browser. See [`portal/README.md`](portal/README.md) for how it gets its data (the VASSAL lobby's `-URL` push and Webswing's admin-console WebSocket). |
