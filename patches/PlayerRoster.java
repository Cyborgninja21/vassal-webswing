...
/*
 * ---------------------------------------------------------------------------
 * vassal-webswing PATCH, against VASSAL 3.7.24's VASSAL/build/module/PlayerRoster.java.
 *
 * Adds one thing: a session the portal opened as a spectator takes the
 * <observer> side without being asked.
 *
 * VASSAL's side picker is an unconditional modal — promptForSide() is reached
 * both from the "Retire / Change Sides" button and from the wizard's finish()
 * step, and its `sideConfig` is a UI configurer with a null preference key, so
 * there is nothing to seed from outside. The one hook that already exists is
 * promptForSide()'s own early return for the observer, which this patch reuses.
 *
 * Observer semantics themselves are stock VASSAL: masked pieces and private
 * hands render hidden. Note that this is client-side rendering, so a spectator
 * is a UX role, not a security boundary — see the plan's §3.7.
 * ---------------------------------------------------------------------------
 */
...
public class PlayerRoster extends AbstractToolbarItem implements CommandEncoder, GameComponent, GameSetupStep, ComponentDescription {
  ...
  protected String promptForSide(String newSide) {
    // vassal-webswing patch: spectators are observers, no question asked. This
    // mirrors the existing early return below for an explicitly chosen observer.
    if (isPortalSpectator()) {
      return OBSERVER;
    }
