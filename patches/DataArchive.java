 ...
 */
/*
 * PATCHED for vassal-webswing (see docker/Dockerfile): negative class-lookup
 * cache. Module code like the 40k module's command encoder probes
 * Class.forName per piece during load; every MISS walks DataArchive's full
 * fallback chain — NFD variant, a classpath resource scan across all shared
 * jars, a .gif variant, a second classpath scan — and builds two exceptions,
 * for thousands of pieces. That held the loading screen's EDT for minutes on
 * a 351 MB module. A name that failed once cannot start existing later except
 * via addExtension, which clears the cache. Everything else is verbatim
 * VASSAL 3.7.24.
 */
